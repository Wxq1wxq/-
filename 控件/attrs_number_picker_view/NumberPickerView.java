package com.carocean.ckxcarsettings.view;
...
/**
 * Created by Carbs.Wang. email : yeah0126@yeah.net github :
...
public class NumberPickerView extends View {
    ...
    private static final int DEFAULT_TEXT_SIZE_NORMAL_SP = 34;

    // default text size of selected item
    private static final int DEFAULT_TEXT_SIZE_SELECTED_SP = 34;

    // default text size of hint text, the middle item's right text
    private static final int DEFAULT_TEXT_SIZE_HINT_SP = 34;

    // distance between selected text and hint text
    private static final int DEFAULT_MARGIN_START_OF_HINT_DP = 8;

    // distance between hint text and right of this view, used in wrap_content
    // mode
    ...
    // default shown items' count, now we display 3 items, the 2nd one is
    // selected
    private static final int DEFAULT_SHOW_COUNT = 3;

    // default items' horizontal padding, left padding and right padding are
    // both 5dp,
    // only used in wrap_content mode
    private static final int DEFAULT_ITEM_PADDING_DP_H = 5;

    // default items' vertical padding, top padding and bottom padding are both
    // 2dp,
    // only used in wrap_content mode
    ...
    // if displayed values' number is less than show count, then this value will
    // be false.
    private boolean mWrapSelectorWheelCheck = true;
    // if you want you set to linear mode from wrap mode when scrolling, then
    // this value will be true.
    private boolean mPendingWrapToLinear = false;

    // if this view is used in same dialog or PopupWindow more than once, and
    // there are several
    // NumberPickerViews linked, such as Gregorian Calendar with MonthPicker and
    // DayPicker linked,
    // set mRespondChangeWhenDetach true to respond onValueChanged callbacks if
    // this view is scrolling
    // when detach from window, but this solution is unlovely and may cause
    // NullPointerException
    // (even i haven't found this NullPointerException),
    // so I highly recommend that every time setting up a reusable dialog with a
    // NumberPickerView in it,
    // please initialize NumberPickerView's data, and in this way, you can set
    // mRespondChangeWhenDetach false.
    ...
    // OnValueChangeListener, OnValueChangeListenerRelativeToRaw and
    // OnScrollListener when view is
    ...
    public interface OnValueChangeListenerRelativeToRaw {
        void onValueChangeRelativeToRaw(NumberPickerView picker, int oldPickedIndex,
                                        int newPickedIndex, String[] displayedValues);
    }
    ...
    private OnValueChangeListener mOnValueChangeListener; // compatible for
    // NumberPicker
    private OnScrollListener mOnScrollListener;// compatible for NumberPicker
    private OnValueChangeListenerInScrolling mOnValueChangeListenerInScrolling;// response
    // onValueChanged
    // in
    // scrolling
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.NumberPickerView_npv_ShowCount) {
                ...
                Log.d("wxq", "initAttr: mTextColorNormal:"+mTextColorNormal);
                ...
                Log.d("wxq", "initAttr: mTextColorSelected:"+mTextColorSelected);
                ...
                mTextSizeNormal = a.getDimensionPixelSize(attr,
                        sp2px(context, DEFAULT_TEXT_SIZE_NORMAL_SP));
            } else if (attr == R.styleable.NumberPickerView_npv_TextSizeSelected) {
                mTextSizeSelected = a.getDimensionPixelSize(attr,
                        sp2px(context, DEFAULT_TEXT_SIZE_SELECTED_SP));
            } else if (attr == R.styleable.NumberPickerView_npv_TextSizeHint) {
                mTextSizeHint = a.getDimensionPixelSize(attr,
                        sp2px(context, DEFAULT_TEXT_SIZE_HINT_SP));
                ...
                mMarginStartOfHint = a.getDimensionPixelSize(attr,
                        dp2px(context, DEFAULT_MARGIN_START_OF_HINT_DP));
            } else if (attr == R.styleable.NumberPickerView_npv_MarginEndOfHint) {
                mMarginEndOfHint = a.getDimensionPixelSize(attr,
                        dp2px(context, DEFAULT_MARGIN_END_OF_HINT_DP));
            } else if (attr == R.styleable.NumberPickerView_npv_ItemPaddingVertical) {
                mItemPaddingVertical = a.getDimensionPixelSize(attr,
                        dp2px(context, DEFAULT_ITEM_PADDING_DP_V));
            } else if (attr == R.styleable.NumberPickerView_npv_ItemPaddingHorizontal) {
                mItemPaddingHorizontal = a.getDimensionPixelSize(attr,
                        dp2px(context, DEFAULT_ITEM_PADDING_DP_H));
                ...
                mRespondChangeInMainThread = a.getBoolean(attr,
                ...
                switch (msg.what) {
                    case HANDLER_WHAT_REFRESH:
                        if (!mScroller.isFinished()) {
                            ...
                            int duration = 0;
                            int willPickIndex;
                            // if scroller finished(not scrolling), then adjust the
                            // position
                            if (mCurrDrawFirstItemY != 0) {// need to adjust
                                ...
                                if (mCurrDrawFirstItemY < (-mItemHeight / 2)) {
                                    ...
                                    mScroller.startScroll(0, mCurrDrawGlobalY, 0, mItemHeight
                                            + mCurrDrawFirstItemY, duration * 3);
                                    ...
                                    mScroller.startScroll(0, mCurrDrawGlobalY, 0, mCurrDrawFirstItemY,
                                            duration * 3);
                                    willPickIndex = getWillPickIndexByGlobalY(mCurrDrawGlobalY
                                            + mCurrDrawFirstItemY);
                                }
                            ...
                            Message changeMsg = getMsg(HANDLER_WHAT_LISTENER_VALUE_CHANGED,
                                    mPrevPickedIndex, willPickIndex, msg.obj);
    ...
    protected void onDetachedFromWindow() {
        ...
        mHandlerThread.quit();
        // These codes are for dialog or PopupWindow which will be used for more
        // than once.
        // Not an elegant solution, if you have any good idea, please let me
        // know, thank you.
        if (mItemHeight == 0)
            return;
        if (!mScroller.isFinished()) {
        ...
        // see the comments on mRespondChangeOnDetach, if mRespondChangeOnDetach
        // is false,
        // please initialize NumberPickerView's data every time setting up
        // NumberPickerView,
        ...
        if (currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach) {
            try {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(NumberPickerView.this, mPrevPickedIndex
                            + mMinValue, currPickedIndex + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(NumberPickerView.this,
    ...
    public void setDisplayedValuesAndPickedIndex(String[] newDisplayedValues, int pickedIndex,
                                                 boolean needRefresh) {
        stopScrolling();
        ...
        if (pickedIndex < 0) {
            throw new IllegalArgumentException(
                    "pickedIndex should not be negative, now pickedIndex is " + pickedIndex);
        }
        ...
        if (mMaxValue - mMinValue + 1 > newDisplayedValues.length) {
            throw new IllegalArgumentException(
                    "mMaxValue - mMinValue + 1 should not be greater than mDisplayedValues.length, now "
                            + "((mMaxValue - mMinValue + 1) is "
                            + (mMaxValue - mMinValue + 1)
                            + " newDisplayedValues.length is "
                            + newDisplayedValues.length
    ...
    /**
     * get the "fromValue" by using getValue(), if your picker's minValue is not
     * 0, make sure you can get the accurate value by getValue(), or you can use
    ...
    /**
     * @param fromValue   need to set the fromValue, can be greater than mMaxValue or
     *                    less than mMinValue
     * @param toValue     the value you want picker to scroll to
     * @param needRespond need Respond to the ValueChange callback When Scrolling,
     *                    default is false
     */
    public void smoothScrollToValue(int fromValue, int toValue, boolean needRespond) {
        int deltaIndex;
        fromValue = refineValueByLimit(fromValue, mMinValue, mMaxValue, mWrapSelectorWheel
                && mWrapSelectorWheelCheck);
        toValue = refineValueByLimit(toValue, mMinValue, mMaxValue, mWrapSelectorWheel
                && mWrapSelectorWheelCheck);
        if (mWrapSelectorWheel && mWrapSelectorWheelCheck) {
            ...
            if (deltaIndex < -halfOneRecycleSize || halfOneRecycleSize < deltaIndex) {
                deltaIndex = deltaIndex > 0 ? deltaIndex - getOneRecycleSize() : deltaIndex
                        + getOneRecycleSize();
            }
        } else {
            deltaIndex = toValue - fromValue;
    ...
    /**
     * simplify the "setDisplayedValue() + setMinValue() + setMaxValue()"
     * process, default minValue is 0, and make sure you do NOT change the
     * minValue.
     *
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange) {
        onScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
        if (oldVal != newVal) {
            if (respondChange == null || !(respondChange instanceof Boolean)
                    || (Boolean) respondChange) {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(NumberPickerView.this, oldVal + mMinValue,
                            newVal + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(NumberPickerView.this,
    ...
    private void scrollByIndexSmoothly(int deltaIndex, boolean needRespond) {
        ...
        if (needRespond) {
            ...
            mHandlerInNewThread.sendMessageDelayed(
                    getMsg(HANDLER_WHAT_REFRESH, 0, 0, new Boolean(needRespond)), duration / 4);
        }
        postInvalidate();
    }
    ...
    public void setValue(int value) {
        Log.d("picker", "setValue: mMinValue:  "+mMinValue+"  mMaxValue "+mMaxValue);
        if (value < mMinValue) {
            throw new IllegalArgumentException(
                    "should not set a value less than mMinValue, value is " + value);
        }
        if (value > mMaxValue) {
            throw new IllegalArgumentException(
                    "should not set a value greater than mMaxValue, value is " + value);
        }
    ...
    public void setPickedIndexRelativeToMin(int pickedIndexToMin) {
        if (0 <= pickedIndexToMin && pickedIndexToMin < getOneRecycleSize()) {
            ...
            correctPositionByDefaultValue(pickedIndexToMin, mWrapSelectorWheel
                    && mWrapSelectorWheelCheck);
            postInvalidate();
        }
    }
    ...
    public void setPickedIndexRelativeToRaw(int pickedIndexToRaw) {
        if (mMinShowIndex > -1) {
            if (mMinShowIndex <= pickedIndexToRaw && pickedIndexToRaw <= mMaxShowIndex) {
                ...
                correctPositionByDefaultValue(pickedIndexToRaw - mMinShowIndex, mWrapSelectorWheel
                        && mWrapSelectorWheelCheck);
                postInvalidate();
            }
        }
    ...
    public int getPickedIndexRelativeToRaw() {
        ...
        if (mCurrDrawFirstItemY != 0) {
            if (mCurrDrawFirstItemY < (-mItemHeight / 2)) {
                willPickIndex = getWillPickIndexByGlobalY(mCurrDrawGlobalY + mItemHeight
                        + mCurrDrawFirstItemY);
            } else {
    ...
    public void setMinAndMaxShowIndex(int minShowIndex, int maxShowIndex, boolean needRefresh) {
        ...
        if (mDisplayedValues == null) {
            ...
            if (minShowIndex < 0) {
                throw new IllegalArgumentException(
                        "minShowIndex should not be less than 0, now minShowIndex is "
                                + minShowIndex);
                ...
                throw new IllegalArgumentException(
                        "minShowIndex should not be greater than (mDisplayedValues.length - 1), now "
                                ...
                                + (mDisplayedValues.length - 1) + " minShowIndex is "
                                + minShowIndex);
            }

            if (maxShowIndex < 0) {
                throw new IllegalArgumentException(
                        "maxShowIndex should not be less than 0, now maxShowIndex is "
                                + maxShowIndex);
            } else if (maxShowIndex > mDisplayedValues.length - 1) {
                throw new IllegalArgumentException(
                        "maxShowIndex should not be greater than (mDisplayedValues.length - 1), now "
                                + "(mDisplayedValues.length - 1) is "
                                + (mDisplayedValues.length - 1) + " maxShowIndex is "
                                + maxShowIndex);
            }
        }
    ...
    /**
     * set the friction of scroller, it will effect the scroller's acceleration
     * when fling
     *
     * @param friction default is ViewConfiguration.get(mContext).getScrollFriction()
     *                 if setFriction(2 *
     *                 ViewConfiguration.get(mContext).getScrollFriction()), the
     *                 friction will be twice as much as before
     */
    ...
    private int getWillPickIndexByGlobalY(int globalY) {
        ...
        int index = getIndexByRawIndex(willPickIndex, getOneRecycleSize(), mWrapSelectorWheel
                && mWrapSelectorWheelCheck);
        if (0 <= index && index < getOneRecycleSize()) {
            ...
            throw new IllegalArgumentException("getWillPickIndexByGlobalY illegal index : " + index
                    + " getOneRecycleSize() : " + getOneRecycleSize() + " mWrapSelectorWheel : "
                    + mWrapSelectorWheel);
        }
    }
    ...
    private void updateDividerAttr() {
        ...
        if (getPaddingLeft() + mDividerMarginL >= mViewWidth - getPaddingRight() - mDividerMarginR) {
            int surplusMargin = getPaddingLeft() + mDividerMarginL + getPaddingRight()
                    + mDividerMarginR - mViewWidth;
            mDividerMarginL = (int) (mDividerMarginL - (float) surplusMargin * mDividerMarginL
                    / (mDividerMarginL + mDividerMarginR));
            mDividerMarginR = (int) (mDividerMarginR - (float) surplusMargin * mDividerMarginR
                    / (mDividerMarginL + mDividerMarginR));
        }
    }
            ...
            case MotionEvent.ACTION_UP:
                if (mFlagMayPress) {
                    ...
                    if (Math.abs(velocityY) > mMiniVelocityFling) {
                        mScroller
                                .fling(0, mCurrDrawGlobalY, 0, -velocityY, Integer.MIN_VALUE,
                                        Integer.MAX_VALUE, limitY(Integer.MIN_VALUE),
    ...
    // first shown item's content index, corresponding to the Index of
    // mDisplayedValued
    ...
    private void calculateFirstItemParameterByGlobalY() {
        ...
        if (mOnValueChangeListenerInScrolling != null) {
            ...
            if (mInScrollingPickedOldValue != mInScrollingPickedNewValue) {
                respondPickedValueChangedInScrolling(mInScrollingPickedOldValue,
    ...
    private int measureWidth(int measureSpec) {
        ...
        if (specMode == MeasureSpec.EXACTLY) {
            ...
            int marginOfHint = Math.max(mWidthOfHintText, mWidthOfAlterHint) == 0 ? 0
                    : mMarginEndOfHint;
            int gapOfHint = Math.max(mWidthOfHintText, mWidthOfAlterHint) == 0 ? 0
                    : mMarginStartOfHint;
            ...
            int maxWidth = Math.max(mMaxWidthOfAlterArrayWithMeasureHint,
                    Math.max(mMaxWidthOfDisplayedValues, mMaxWidthOfAlterArrayWithoutMeasureHint)
                            + 2
                            * (gapOfHint + Math.max(mWidthOfHintText, mWidthOfAlterHint)
    ...
    private void drawContent(Canvas canvas) {
        ...
        int textColor;
        float textSize;
        float fraction = 0f;// fraction of the item in state between normal and
        // selected, in[0, 1]
        ...
        for (int i = 0; i < mShowCount + 1; i++) {
            ...
            Log.d("wxq", "drawContent: textColor:"+textColor);
            ...
            if (0 <= index && index < getOneRecycleSize()) {
                ...
                if (mTextEllipsize != null) {
                    str = TextUtils.ellipsize(str, mPaintText, getWidth() - 2
                            * mItemPaddingHorizontal, getEllipsizeType());
                }
                canvas.drawText(str.toString(), mViewCenterX, y + mItemHeight / 2
                        + textSizeCenterYOffset, mPaintText);
            } else if (!TextUtils.isEmpty(mEmptyItemHint)) {
                canvas.drawText(mEmptyItemHint, mViewCenterX, y + mItemHeight / 2
                        + textSizeCenterYOffset, mPaintText);
            }
        }
    }
    ...
    private TextUtils.TruncateAt getEllipsizeType() {
        if (mTextEllipsize.equals(TEXT_ELLIPSIZE_START)) {
            return TextUtils.TruncateAt.START;
        } else if (mTextEllipsize.equals(TEXT_ELLIPSIZE_MIDDLE)) {
            return TextUtils.TruncateAt.MIDDLE;
        } else if (mTextEllipsize.equals(TEXT_ELLIPSIZE_END)) {
    ...
    private void drawLine(Canvas canvas) {
        if (mShowDivider) {
            canvas.drawLine(getPaddingLeft() + mDividerMarginL, dividerY0, mViewWidth
                    - getPaddingRight() - mDividerMarginR, dividerY0, mPaintDivider);
            canvas.drawLine(getPaddingLeft() + mDividerMarginL, dividerY1, mViewWidth
                    - getPaddingRight() - mDividerMarginR, dividerY1, mPaintDivider);
        }
    }
    ...
    private void drawHint(Canvas canvas) {
        if (TextUtils.isEmpty(mHintText))
            return;
        canvas.drawText(mHintText, mViewCenterX + (mMaxWidthOfDisplayedValues + mWidthOfHintText)
                        / 2 + mMarginStartOfHint, (dividerY0 + dividerY1) / 2 + mTextSizeHintCenterYOffset,
                mPaintHint);
    }
    ...
    private void updateMaxWidthOfDisplayedValues() {
        ...
        mMaxWidthOfAlterArrayWithMeasureHint = getMaxWidthOfTextArray(
                mAlterTextArrayWithMeasureHint, mPaintText);
        mMaxWidthOfAlterArrayWithoutMeasureHint = getMaxWidthOfTextArray(
                mAlterTextArrayWithoutMeasureHint, mPaintText);
    ...
    private void updateMaxHeightOfDisplayedValues() {
        float savedTextSize = mPaintText.getTextSize();
        mPaintText.setTextSize(mTextSizeSelected);
        mMaxHeightOfDisplayedValues = (int) (mPaintText.getFontMetrics().bottom
                - mPaintText.getFontMetrics().top + 0.5);
        mPaintText.setTextSize(savedTextSize);
    }
